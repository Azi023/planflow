...
function ExportDropdown({ onExcel, onPptx, onPrint, exportingExcel, exportingPptx }: {
  onExcel: () => void; onPptx: () => void; onPrint: () => void;
  exportingExcel: boolean; exportingPptx: boolean;
}) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen(v => !v)}
        className="bg-white border border-[#DBDFE9] text-[#4B5675] rounded-lg px-4 py-2.5 text-sm font-medium hover:bg-[#F9F9F9] transition-colors flex items-center gap-2"
      >
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
        Export
        <svg viewBox="0 0 24 24" className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6" /></svg>
      </button>
      {open && (
        <div className="absolute left-0 top-full mt-1 w-48 bg-white rounded-lg border border-[#E1E3EA] py-1 z-20" style={{ boxShadow: '0px 10px 30px rgba(0,0,0,0.12)' }}>
          <button onClick={() => { setOpen(false); onExcel(); }} disabled={exportingExcel} className="w-full text-left px-4 py-2.5 text-sm text-[#071437] hover:bg-[#F9F9F9] transition-colors flex items-center gap-2 disabled:opacity-60">
            <span className="text-[#2E7D32]">📊</span> {exportingExcel ? 'Generating…' : 'Excel (.xlsx)'}
          </button>
          <button onClick={() => { setOpen(false); onPptx(); }} disabled={exportingPptx} className="w-full text-left px-4 py-2.5 text-sm text-[#071437] hover:bg-[#F9F9F9] transition-colors flex items-center gap-2 disabled:opacity-60">
            <span className="text-[#D05A1E]">📑</span> {exportingPptx ? 'Generating…' : 'PowerPoint (.pptx)'}
          </button>
          <div className="my-1 border-t border-[#F1F1F4]" />
          <button onClick={() => { setOpen(false); onPrint(); }} className="w-full text-left px-4 py-2.5 text-sm text-[#071437] hover:bg-[#F9F9F9] transition-colors flex items-center gap-2">
